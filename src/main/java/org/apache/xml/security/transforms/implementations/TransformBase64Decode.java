...
public class TransformBase64Decode extends TransformSpi {
    ...
    protected XMLSignatureInput enginePerformTransform(
        ...
        if (input.isElement()) {
            ...
            if (os == null) {
                final byte[] decodedBytes = XMLUtils.decode(sb.toString());
                final XMLSignatureInput output = new XMLSignatureInput(decodedBytes);
                output.setSecureValidation(secureValidation);
                return output;
            }
            final byte[] bytes = XMLUtils.decode(sb.toString());
            os.write(bytes);
            ...
            if (os == null) {
                final byte[] base64Bytes = input.getBytes();
                final byte[] decodedBytes = XMLUtils.decode(base64Bytes);
                final XMLSignatureInput output = new XMLSignatureInput(decodedBytes);
                output.setSecureValidation(secureValidation);
                return output;
            }
            if (input.isByteArray() || input.isNodeSet()) {
                final byte[] bytes = XMLUtils.decode(input.getBytes());
                os.write(bytes);
            } else {
                final byte[] inputBytes = JavaUtils.getBytesFromStream(input.getOctetStreamReal());
                final byte[] bytes = XMLUtils.decode(inputBytes);
                os.write(bytes);
            }
            final XMLSignatureInput output = new XMLSignatureInput((byte[])null);
            output.setSecureValidation(secureValidation);
