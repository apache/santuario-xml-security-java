...
public class KeyTestUtils {
    ...
    public static KeyPair generateKeyPair(KeyUtils.KeyType keyType) throws NoSuchAlgorithmException, InvalidAlgorithmParameterException, InvalidParameterSpecException {
        Provider provider = JDKTestUtils.isAlgorithmSupported(keyType.name()) ? null : JDKTestUtils.getAuxiliaryProvider();
        KeyPairGenerator keyPairGenerator;

        switch (keyType.getAlgorithm()){
            case EC:{
                keyPairGenerator = provider == null ? KeyPairGenerator.getInstance(keyType.getAlgorithm().getJceName()) :
            ...
            default:
                throw new IllegalStateException("Unexpected value: " + keyType.getAlgorithm());
    ...
    public static byte[] getKeyResourceAsByteArray(String fileName) throws IOException {
        InputStream keyIS = getKeyResourceAsInputStream(fileName);
        byte[] keyBytes = new byte[keyIS.available()];
        keyIS.read(keyBytes);
